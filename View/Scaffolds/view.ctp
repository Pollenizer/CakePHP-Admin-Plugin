...
<div class="span9">
            ...
            <?php if (!empty($associations['belongsTo'])): ?>
                <?php foreach ($associations['belongsTo'] as $_alias => $_details): ?>
                    <?php if ($_field === $_details['foreignKey']): ?>
                        ...
                        <dd><?php echo $this->Html->link(${$singularVar}[$_alias][$_details['displayField']], array('plugin' => 'admin', 'controller' => 'admin_' . $_details['controller'], 'action' => 'view', ${$singularVar}[$_alias][$_details['primaryKey']])); ?></dd>
                        <?php break; ?>
                    <?php endif; ?>
                <?php endforeach; ?>
            <?php endif; ?>
        ...
        <?php foreach ($associations['hasOne'] as $_alias => $_details): ?>
            <div>
                ...
                <div class="actions well">
                    <?php echo $this->Html->link(__d('cake', 'Edit %s', Inflector::humanize(Inflector::underscore($_alias))), array('plugin' => 'admin', 'controller' => 'admin_' . $_details['controller'], 'action' => 'edit', ${$singularVar}[$_alias][$_details['primaryKey']]), array('class' => 'btn primary')); ?>
                </div>
            </div>
        <?php endforeach; ?>
    <?php endif; ?>
    ...
    <?php foreach ($relations as $_alias => $_details): ?>
        ...
        <div>
                    ...
                    <tbody>
                        <?php foreach (${$singularVar}[$_alias] as ${$otherSingularVar}): ?>
                            <tr>
                                ...
                                <?php endforeach; ?>
                                <td><?php echo $this->Html->link(__d('cake', 'View'), array('plugin' => 'admin', 'controller' => 'admin_' . $_details['controller'], 'action' => 'view', ${$otherSingularVar}[$_details['primaryKey']]), array('class' => 'btn btn-info')); ?></td>
                                <td><?php echo $this->Html->link(__d('cake', 'Edit'), array('plugin' => 'admin', 'controller' => 'admin_' . $_details['controller'], 'action' => 'edit', ${$otherSingularVar}[$_details['primaryKey']]), array('class' => 'btn btn-warning')); ?></td>
                                <td><?php echo $this->Html->link(__d('cake', 'Delete'), array('plugin' => 'admin', 'controller' => 'admin_' . $_details['controller'], 'action' => 'delete', ${$otherSingularVar}[$_details['primaryKey']]), array('class' => 'btn btn-danger'), __d('cake', 'Are you sure you want to delete %s %s?', Inflector::humanize(Inflector::underscore($_alias)), ${$otherSingularVar}[$_details['primaryKey']])); ?></td>
                            </tr>
                        <?php endforeach; ?>
                    </tbody>
                </table>
            <?php endif; ?>
            <div class="well">
                <?php echo $this->Html->link(__d('cake', "New %s", Inflector::humanize(Inflector::underscore($_alias))), array('plugin' => 'admin', 'controller' => 'admin_' . $_details['controller'], 'action' => 'add'), array('class' => 'btn btn-primary')); ?>
            </div>
        </div>
    <?php endforeach;?>
</div>
